...
typedef struct end_point {
    unsigned int    val;
    int		    type;   // 1: by rule low, 2: by rule high, 3: by both
} epoint_t;

epoint_t	epoints[FIELDS][MAXRULES*2];	
...
void init_epoint_hash()
{
    ...
    for (f = 0; f < FIELDS; f++) {
	epoint_hash[f] = (epoint_t ***) malloc(hash_tab_sizes[f]*sizeof(epoint_t**));
	...
	for (i = 0; i < hash_tab_sizes[f]; i++) {
	    ...
	    epoint_hash[f][i] = (epoint_t **) malloc(hash_entry_size[f][i]*sizeof(epoint_t*));
...
// add the pointer of a new end-point to the hash for future end-point lookup
void add_epoint_hash(epoint_t *point, int f)
{
    unsigned int    h, n, i;

    h = point->val % hash_tab_sizes[f];
    n = hash_entry_num[f][h];

    if (n == hash_entry_size[f][h]) {
	hash_entry_size[f][h] <<= 1;
	epoint_hash[f][h] = (epoint_t **) realloc(epoint_hash[f][h], hash_entry_size[f][h]*sizeof(epoint_t *));
    }
...
epoint_t* lookup_epoint_hash(unsigned int point_val, int f)
{
    unsigned int    h, n, i;

    h = point_val % hash_tab_sizes[f];
    n = hash_entry_num[f][h];
    for (i = 0; i < n; i++) {
	if (epoint_hash[f][h][i]->val == point_val)
	    return epoint_hash[f][h][i];
    }

    // no hit on existing end-points
    return NULL;
}
...
{
    if (((epoint_t *)p)->val > ((epoint_t *)q)->val)
	return 1;
    else if (((epoint_t *)p)->val < ((epoint_t *)q)->val)
	return -1;
    else 
	return 0;
}
...
void sort_epoints(int field)
{
    ...
    qsort(epoints[field], num_epoints[field], sizeof(epoint_t), point_cmp);
}
...
{
    epoint_t	    *p = epoints[f], *point;
    unsigned int    point_val;
    uint16_t	    i, r;
    int		    hit;
    ...
    for (i = 0; i < nrules; i++) {
	r = rules[i];
	point_val = ruleset[r].field[f].low;
	point = lookup_epoint_hash(point_val, f);
	if (point == NULL) {
	    point = &p[num_epoints[f]];
	    point->val = point_val;
	    point->type = 1;
	    add_epoint_hash(point, f);
	    num_epoints[f]++;
	} else {
	    point->type |= 1;
	}
	
	if (ruleset[r].field[f].high == point_val) {
	    // rule r begins & ends at a single point, and the type of the point should be both (3)
	    point->type = 3;
	} else {
	    point_val = ruleset[r].field[f].high;
	    point = lookup_epoint_hash(point_val, f);
	    if (point == NULL) {
		point = &p[num_epoints[f]];
		point->val = point_val;
...
int create_one_flow()
{
    ...
    flows[num_flows].sip = epoint_stack[0].val;
    flows[num_flows].dip = epoint_stack[1].val;
    flows[num_flows].proto = epoint_stack[2].val;
    flows[num_flows].sp = epoint_stack[3].val;
    flows[num_flows].dp = epoint_stack[4].val;
...
int process_field(int f)
{
	...
	for (i = 0; i < num_epoints[f]; i++) {
	    point_val = epoints[f][i].val;
	    //TODO: only collecting rules at end-points is not enough, some intervals may have
	    // rules than their enclosing end-points
	    collect_field_epoint_rules(f, point_val);
	    epoint_stack[f].val = point_val;
...
int calc_rule_type(uint16_t rule, int f)
{
    unsigned int    *key;
    epoint_t	    *p1, *p2; 
    int		    n;
    ...
    p1 = bsearch(key, epoints[f], num_epoints[f], sizeof(epoint_t), point_cmp);
    key = &ruleset[rule].field[f].high;
    p2 = bsearch(key, epoints[f], num_epoints[f], sizeof(epoint_t), point_cmp);

    if (p2 - p1 == 0)	// rule is local on f, as it is a single point on field f
	return 1; 
    if (p2 - p1 == 1) {	// rule does not cross any end-point on field f
	// if either of its two end-points is both a begin and an end point, rule is not local on f
	// as it may appear in two CBMs, one CBM for its end-point, and another CBM for its interval
	if (p1->type == 3 || p2->type == 3)
	    return 0;
	else
	    return 1;
    } else
	return 0;	// rule cannot be a local rule as it crosses end-points
}


// statistcis on the number of rules crossing different numbers of endpoints (2^0, 2^1, 2^2, ...)
// 0: a local rule on field f (does not cross any end-point on a field f)
// 1: at least cross one end-point on field f
void calc_ruleset_types()
{
    int		f, i, n, rule_sizes[FIELDS][32];

    for (f = 0; f < FIELDS; f++)
	rule_types[f] = (int *) calloc(numrules, sizeof(int));
	...
	sort_epoints(f);
	for (i = 0; i < num_field_rules[f]; i++)
	    rule_types[f][i] = calc_rule_type(field_rules[f][i], f);
    }
}
